<template>
  ...
  <div class="bg-white dark:bg-gray-800 p-3 rounded-lg dark:border dark:border-gray-700">
    ...
    <!-- Loading State -->
    <div v-if="accountStore.loading && accountStore.items.length === 0" class="text-center py-12">
      <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-gray-600"></div>
      <p class="mt-4 text-gray-600 dark:text-gray-400">Loading accounts...</p>
    </div>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="(data, index) in displayedItems" :key="data.id"
              ...
              <td class="px-6 py-3">
                ...
                <div v-if="data.company_name" class="text-xs text-gray-500 dark:text-gray-400">
                  {{ data.company_name }}
                </div>
              </td>
            ...
            <!-- Empty State -->
            <tr v-if="displayedItems.length === 0 && !accountStore.loading"
              ...
              <td colspan="8" class="px-6 py-12 text-center">
                <div class="text-gray-500 dark:text-gray-400">
                  ...
                  <p class="mt-1">Get started by adding your first account</p>
                  <button @click="openCreateModal"
    ...
    <div v-if="accountStore.pagination.total > 0" class="mt-6">
      <Pagination :total-items="accountStore.pagination.total" :per-page="accountStore.pagination.per_page"
        :current-page="accountStore.pagination.current_page" :last-page="accountStore.pagination.last_page"
    ...
    <form @submit.prevent="handleSubmit" class="px-6 py-3">
      <div class="grid grid-cols-1 md:grid-cols-1 2xl:grid-cols-1 gap-4">
        ...
        <div>
          ...
          <p v-if="accountStore.validationErrors.status" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ accountStore.validationErrors.status[0] }}
          </p>
        </div>
  ...
  <BaseModal v-if="viewingAccount" :show="showViewModal" maxWidth="xl" title="Account Details" @close="closeViewModal">
    <div class="px-6 py-3">
      <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">{{ viewingAccount.account_name }}</h2>
        <p class="text-gray-600 dark:text-gray-400">{{ viewingAccount.account_number }}</p>
      </div>
