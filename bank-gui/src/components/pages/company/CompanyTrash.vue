<template>
    ...
    <div class="bg-white dark:bg-gray-700 rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <!-- Loading -->
          <TableLoadingState v-if="companyStore.loading" :is-loading="true" item-name="companies"
            loading-text="Loading deleted companies..." />

          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <!-- Empty -->
            <TableLoadingState v-if="!companyStore.loading && companyStore.items.length === 0" :is-loading="false"
              :isEmpty="true" :has-search="!!searchQuery" item-name="deleted companies" empty-icon="fas fa-building"
              empty-title="No deleted companies" :empty-message="searchQuery
                ? 'No deleted companies match your search.'
                : 'There are no deleted companies.'" @clear-search="clearSearch" />

            <!-- Rows -->
            <tr v-else v-for="(data, index) in companyStore.items" :key="data.id"
              ...
              <td class="px-6 py-3 text-sm text-right space-x-2">
                <button @click="confirmRestore(data)" class="text-green-600 hover:text-green-800">
                  <i class="fas fa-undo"></i>
                </button>

                <button @click="confirmPermanentDelete(data)" class="text-red-600 hover:text-red-800">
  ...
  <BaseModal :show="showDeleteModal" title="Permanently Delete Company" @close="closeDelete">
