<template>
  ...
  <div class="bg-white dark:bg-gray-800 p-3 rounded-lg dark:border dark:border-gray-700">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
      ...
      <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
        <div class="relative flex-grow max-w-md">
          <input v-model="searchQuery" @input="handleSearch" type="search" placeholder="Search deleted users..."
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <TableLoadingState v-if="userStore.loading" :is-loading="true" :total-items="userStore.totalItems"
            :item-name="'users'" loading-text="Loading deleted users..."
            loading-subtext="Fetching your deleted user data" />

          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <!-- Empty State -->
            <TableLoadingState v-if="userStore.items.length === 0 && !userStore.loading" :is-loading="false"
              :isEmpty="true" :has-search="!!searchQuery" :item-name="'deleted users'" empty-icon="fas fa-users-slash"
    ...
    <div v-if="userStore.totalItems > 0" class="mt-6">
      <Pagination :total-items="userStore.totalItems" :per-page="userStore.perPage"
        :current-page="userStore.currentPage" :last-page="userStore.lastPage" :is-loading="userStore.loading"
  ...
  <BaseModal :show="showRestoreModal" title="Restore User" maxWidth="lg" @close="closeRestoreModal">
    <div class="px-6 py-3 text-center">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
        Are you sure you want to restore <strong>{{ restoringUser?.first_name }} {{ restoringUser?.last_name
          }}</strong>?
      </p>
      <div class="flex justify-center space-x-3">
        ...
        <button @click="handleRestore" :disabled="userStore.loading"
  ...
  <BaseModal :show="showPermanentDeleteModal" title="Permanently Delete User" maxWidth="lg"
    ...
    <div class="px-6 py-3 text-center">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
        Are you sure you want to permanently delete <strong>{{ permanentDeletingUser?.first_name }} {{
          permanentDeletingUser?.last_name }}</strong>?
      </p>
      <div class="flex justify-center space-x-3">
        <button @click="closePermanentDeleteModal"
          class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          Cancel
        </button>
        <button @click="handlePermanentDelete" :disabled="userStore.loading"
