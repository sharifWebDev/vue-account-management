<template>
  <div class="mb-4">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-lg font-semibold text-gray-700 dark:text-white">Banks</h1>
      </div>
      <Breadcrumb />
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-gray-800 p-3 rounded-lg dark:border dark:border-gray-700">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
      ...
      <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
        <div class="relative flex-grow max-w-md">
          <input v-model="searchQuery" @input="handleSearch" type="search" placeholder="Search by bank name..."
    ...
    <div class="bg-white dark:bg-gray-700 rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 relative">
          <SortableTableHeader :columns="tableColumns" :sort-by="bankStore.sortBy"
            :sort-direction="bankStore.sortDirection" :show-actions-column="true" :show-sl-column="true"
            @sort="sortByColumn" />

          <!-- Loading State - Inside table body -->
          <TableLoadingState v-if="bankStore.loading" :is-loading="true" :total-items="bankStore.totalItems"
            :item-name="'banks'" loading-text="Loading banks..." loading-subtext="Fetching your bank data" />

          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">

            <!-- Empty State -->
            <TableLoadingState v-if="bankStore.items.length === 0" :is-loading="false" :isEmpty="true"
              :has-search="!!searchQuery" :item-name="'banks'" empty-icon="fas fa-university"
              empty-title="No banks found"
              :empty-message="searchQuery ? 'No banks match your search criteria.' : 'Get started by creating your first bank.'"
              :create-button-text="'Add First Bank'" @create="openCreateModal" @clear-search="clearSearch" />
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-300">
                      {{ data.total_branch ?? 0 }} {{ data.total_branch > 1 ? 'Branches' : 'Branch' }}
                    </div>
                  </div>
                </div>
              </td>
    ...
    <div v-if="bankStore.pagination.total > 0" class="mt-6">
      <Pagination v-if="bankStore.totalItems > 0" :total-items="bankStore.totalItems" :per-page="bankStore.perPage"
        :current-page="bankStore.currentPage" :last-page="bankStore.lastPage" :is-loading="bankStore.isLoading"
        @change-page="handlePageChange" @change-per-page="handlePerPageChange" />
    </div>
  </div>

  <!-- Create/Edit Modal -->
  <BaseModal v-if="isModalOpen" :show="isModalOpen" :title="modalMode === 'create' ? 'Create Bank' : 'Edit Bank'"
      ...
      <div class="grid grid-cols-1 md:grid-cols-1 2xl:grid-cols-1 gap-4">
        ...
        <div>
          ...
          <p v-if="bankStore.validationErrors.bank_name" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.bank_name[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.address" type="text" placeholder="Enter address"
            ...
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.address }">
          <p v-if="bankStore.validationErrors.address" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.address[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.phone" type="text" placeholder="Enter phone number"
            ...
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.phone }">
          <p v-if="bankStore.validationErrors.phone" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.phone[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.mobile" type="text" placeholder="Enter mobile number"
            ...
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.mobile }">
          <p v-if="bankStore.validationErrors.mobile" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.mobile[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.fax" type="text" placeholder="Enter fax number" @change="clearValidationError('fax')"
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.fax }">
          <p v-if="bankStore.validationErrors.fax" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.fax[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.email" type="email" placeholder="Enter email address"
            ...
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.email }">
          <p v-if="bankStore.validationErrors.email" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.email[0] }}
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.website" type="url" placeholder="Enter website URL"
            ...
            class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
            :class="{ 'border-red-500': bankStore.validationErrors.website }">
          <p v-if="bankStore.validationErrors.website" class="text-xs text-red-600 dark:text-red-400 mt-1">
            {{ bankStore.validationErrors.website[0] }}
          </p>
        </div>
        ...
        <div v-if="Object.keys(bankStore.validationErrors).length > 0"
          ...
          <ul class="text-xs text-red-500 dark:text-red-400 space-y-1">
            <li v-for="(errors, field) in bankStore.validationErrors" :key="field">
              • {{ errors[0] }}
            </li>
          </ul>
        </div>
        ...
        <div class="mt-6 flex justify-end space-x-3 col-span-1 md:col-span-2 2xl:col-span-3">
          ...
          <button type="submit" :disabled="bankStore.loading" :class="[
            'px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors',
            bankStore.loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gray-600 hover:bg-gray-700'
          ]">
            <span v-if="bankStore.loading">
  ...
  <BaseModal v-if="viewingBank" :show="showViewModal" maxWidth="xl" title="Bank Details" @close="closeViewModal">
    <div class="px-6 py-3">
      <div class="text-center mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ viewingBank.bank_name }}</h2>
        <p class="text-gray-600 dark:text-gray-400">{{ viewingBank.email || 'No email provided' }}</p>
      </div>

      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">ID</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.id }}</p>
        </div>
        <div>
          ...
          <div class="mt-1">
            <span :class="[
              'px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full',
              viewingBank.status === 1 || viewingBank.status === true
                ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300'
                : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300'
            ]">
              {{ viewingBank.status === 1 || viewingBank.status === true ? 'Active' : 'Inactive' }}
            </span>
          </div>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Address</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.address || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Phone</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.phone || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Mobile</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.mobile || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Fax</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.fax || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Email</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.email || 'N/A' }}</p>
        </div>
        <div>
          <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Website</label>
          <p class="text-sm text-gray-900 dark:text-white">{{ viewingBank.website || 'N/A' }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="px-6 py-3">
      <div class="text-center">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
          Are you sure you want to delete <strong>{{ deletingBank?.bank_name }}</strong>? This action cannot be
          undone.
        </p>
      </div>
      <div class="flex justify-center space-x-3">
        ...
        <button @click="handleDelete" :disabled="bankStore.loading" :class="[
          'px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors',
          bankStore.loading ? 'bg-red-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'
        ]">
          <span v-if="bankStore.loading">
            <i class="fas fa-spinner fa-spin mr-2"></i>
            Deleting...
...
const calculateRowNumber = (index) => {
  ...
  return (bankStore.pagination.current_page - 1) * bankStore.pagination.per_page + index + 1;
};
...
const handleSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    await bankStore.search(searchQuery.value);
  }, 500);
};
...
const handleSubmit = async () => {
  ...
  if (modalMode.value === 'create') {
    result = await bankStore.create(formDataToSend);
  } else {
    formDataToSend.append('_method', 'PUT');
    result = await bankStore.update(currentBank.value.id, formDataToSend);
  }
...
const handleDelete = async () => {
  if (deletingBank.value) {
    const result = await bankStore.deleteItem(deletingBank.value.id);
    if (result.success) {
      closeDeleteModal();
      await bankStore.get();
    }
...
const toggleStatus = async (item, field) => {
  try {
    await bankStore.toggleStatus(item, field);
    await bankStore.get();
  } catch (error) {
