      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="flex flex-wrap items-end gap-4">
          ...
          <div class="flex-1 min-w-[220px]">
            ...
            <div class="relative">
              <input v-model="filters.search" type="search" placeholder="Search transactions..."
                class="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white" />
          ...
          <div class="flex-1 min-w-[200px]">
            ...
            <input v-model="filters.account_number" type="text" placeholder="Account number"
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <template v-if="form.type === 'Transfer'">
            ...
            <div>
              ...
              <input v-model="form.transfer_amount" type="number" step="0.01" min="0" placeholder="0.00"
                class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"
                required>
            </div>
            ...
            <div>
              ...
              <select v-model="form.to_account_id"
                ...
                required>
                <option value="">-- Select Account --</option>
                <option v-for="account in accounts.filter(acc => acc.id !== form.account_id)" :key="account.id"
                  :value="account.id">
                  {{ account.account_name }} ({{ account.account_number }})
                </option>
              </select>
          ...
          <!-- Related Party Fields (for Deposit/Withdraw) -->
          <template v-if="form.type !== 'Transfer'">
            <div v-if="form.type === 'Deposit'">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Receive From *
          ...
          <div class="mt-4 col-span-3">
            ...
            <textarea v-model="form.details" rows="3" placeholder="Enter transaction details"
              class="w-full pl-4 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-400 dark:bg-gray-700 dark:text-white"></textarea>
          </div>

          <!-- Bounce Transaction -->
          <div v-if="modalMode === 'edit'" class="col-span-3">
          ...
          <!-- Error Messages -->
          <div v-if="error"
            class="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg col-span-3">
      ...
      <div v-if="viewingTransaction" class="p-6">
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            <!-- Balance -->
            <div>
              <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mt-5">Transaction
                Amount</label>
              <p :class="[
            ...
            <div>
              ...
              <p class="text-lg font-bold text-blue-600 dark:text-blue-400 mt-1">
                {{ formatCurrency(viewingTransaction.closing_balance) }}
              </p>
            </div>
          </div>
...
const form = ref({
  type: 'Deposit',
  account_id: '',
  to_account_id: '', // To Account (for transfers)
...
// Add a computed property or watcher to prevent same account transfers
watch(() => form.value.account_id, (newVal) => {
  if (form.value.type === 'Transfer' && newVal === form.value.to_account_id) {
    form.value.to_account_id = '';
    toast.warning('Cannot transfer to the same account');
  }
});

watch(() => form.value.to_account_id, (newVal) => {
  if (form.value.type === 'Transfer' && newVal === form.value.account_id) {
    toast.warning('Cannot transfer to the same account');
  }
});
...
const saveTransaction = async () => {
  ...
  try {
    ...
    if (form.value.type === 'Transfer') {
      ...
      if (!form.value.account_id || !form.value.to_account_id || !form.value.transfer_amount) {
      ...
      const withdrawData = {
        ...
        withdraw: form.value.transfer_amount,
        details: `Transfer to account: ${form.value.details}`,
        status: 1,
        is_transfer: 1,
      ...
      if (modalMode.value === 'create') {
        ...
        Object.entries(withdrawData).forEach(([key, value]) => {
          if (value !== null && value !== undefined && value !== '') {
            withdrawFormData.append(key, value);
          }
        });

        await transactionStore.create(withdrawFormData);
        ...
        withdrawFormData.append('_method', 'PUT');
        await transactionStore.update(viewingTransaction.value.id, withdrawFormData);
      }
