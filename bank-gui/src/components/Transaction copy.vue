          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="(transaction, index) in transactions" :key="transaction.id"
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <button @click="toggleStatus(transaction)" :class="[
...
const saveTransactionOld = async () => {
  saving.value = true;
  try {
    ...
    const hasBounce = data.has_bounce;
    delete data.has_bounce;
    ...
    if (modalMode.value === 'create') {
      ...
      await transactionStore.update(viewingTransaction.value.id, data);
    }
...
const toggleStatus = async (transaction) => {
  try {
    ...
    await transactionStore.changeStatus(transaction.id, newStatus ? 'active' : 'inactive');
    toast.success(`Transaction ${newStatus ? 'activated' : 'deactivated'}`);
    await applyFilters();
  } catch (error) {
